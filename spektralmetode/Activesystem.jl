...
function interpolatderivpolar(polarisatincoefficients, y, n)
    ...
    spilnes = [Spline1D(y, polarisatincoefficients[:, i, n]) for i in 1:2]
...
function calculate_ϕ(polarisationtioncoefficients, N, Lx, x, m)
    ...
    ϕ_step = zeros(2*N+1, m-2)

    for j in 2:m-1
        
        for i in 0:N
            ϕ_step[:, j-1] += polarisationtioncoefficients[j, 1, i+1] * cos.(((2 * π * i)/Lx)*x ) + polarisationtioncoefficients[j, 2, i+1] * sin.(((2 * π * i)/Lx)*x )
        end
    end
...
function runge_kutta_polarization(polarisatincoefficients, Dmatrix, N, factor, λ, y, dt)
    ...
    Dmatrix[1] = polarisationtimederivs(polarisatincoefficients, Dmatrix[1], N, factor, λ, y)

    Dmatrix[2] = polarisationtimederivs(polarisatincoefficients + dt/2 * Dmatrix[1], Dmatrix[2], N, factor, λ, y)

    Dmatrix[3] = polarisationtimederivs(polarisatincoefficients + dt/2 * Dmatrix[2], Dmatrix[3], N, factor, λ, y)

    Dmatrix[4] = polarisationtimederivs(polarisatincoefficients + dt * Dmatrix[3], Dmatrix[4], N, factor, λ, y)
...
function runsystem(N::Int, dy::Float64, Lx::Int, Ly::Int, F::Float64, T::Int, dt::Float64, λ::Float64)
    ...
    for i in 2:T
        ...
        polarisationtioncoefficients = runge_kutta(polarisationtioncoefficients, Dmatrix, N, factors, λ, y, dt)

        ϕ[:, 2:end-1, i] = calculate_ϕ(polarisationtioncoefficients, N, Lx, x, m)
...
x, y, ϕ = runsystem(10, 1., 40, 20, 10. ,10000, 0.001, 10.)


GLMakie.arrows(x, y, cos.(ϕ[:, :, 2]), sin.(ϕ[:, :, 2]), arrowsize = 10, lengthscale = 0.1)
